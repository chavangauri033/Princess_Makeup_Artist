...
<!--==============================Content=================================-->
<center>
	<u><h1 align=center>Monthly Report</h1></u>
	<form method=post>
		select month <input type=text name=mon> select year <input type=text name=yea><br><br>
		<input type=submit name=btnsub>
	</form>


	<h1 align=center>Product Order</h1>
 	<table class="table">
			...
			<?php
				if(isset($_POST["btnsub"]))
				{
					...
					$rs=mysqli_query($cn,"select * from product_order where month(Orderdate)=$m and year(Orderdate)=$y");
...
<br><br>
<h1 align=center>Makeup Order Report</h1>
 	<table class="table">
    		<thead bgcolor=#BDBDBD>
      			<tr>
			...
			<?php
				if(isset($_POST["btnsub"]))
				{
					...
					$rs=mysqli_query($cn,"select * from manage_makeuporder where month(Mcurrent_Date)=$m and year(Mcurrent_Date)=$y");
					while($a=mysqli_fetch_array($rs))
					{
		       				$id=$a['Makeup_Id'];
						$txtnm=$a['Ure_Name'];
						...
						echo "<td><center><h5>$id</h5></center></td><td><center><h5>$txtnm</h5></center></td><td><center><h5>$txtadd</h5></center></td>
							...
							<td><center><h5>$txtdt</h5></center></td></td><td><center><h5>$txtod</h5></center></td><td><center><h5>$txtpayment</h5></center></td>";
						echo "</tr>";
					}
				}
			?>
...
<br><br>
<h1 align=center>Admission Report</h1>
 	<table class="table">
    		<thead bgcolor=#BDBDBD>
      			<tr>
			...
			<?php
				if(isset($_POST["btnsub"]))
				{
					...
					$rs=mysqli_query($cn,"select * from admission where month(Admission_date)=$m and year(Admission_date)=$y");
					while($a=mysqli_fetch_array($rs))
					{
		       				$id=$a['Admission_Id'];
						$txtnm=$a['St_Name'];
